...
public struct Series {
    ...
    @discardableResult public func categories(sheet: Worksheet, range: Range) -> Series {
        let _ = sheet.name.withCString {
            chart_series_set_categories(lxw_chart_series, $0, range.row, range.col, range.row2, range.col2)
        }
        return self
    }
    ...
    @discardableResult public func trendline(color: Color = .black, width: Float = 2.25, dash_type: Int, transparency: Int = 0, hide: Bool = false) -> Series {
